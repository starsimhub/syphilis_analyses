...
# OLD/ UNSUSED
############
# Partner acquisition rates - superseded by preferred partner matching
# first_part_acq_f=dict(  # Number of new partners per year for women with 0 partners
#     teens=np.array([0.4, 0.5, 0.45]),   # Girls aged 15-19: F0, F1, F2
#     young=np.array([0.1, 0.2, 0.2]),    # Young women aged 20-24: F0, F1, F2
#     adult=np.array([0.02, 0.05, 0.1]),  # Women aged 25+: F0, F1, F2
# ),
# extra_part_acq_f=dict(  # Number of new partners per year for women with 1+ partners
#     teens=np.array([0, 0.05, 0.1]),     # Girls aged 15-19: F0, F1, F2
#     young=np.array([0, 0.02, 0.05]),    # Young women aged 20-24: F0, F1, F2
#     adult=np.array([0, 0.01, 0.01]),  # Women aged 25+: F0, F1, F2
# ),

# Age and risk mixing - superseded by embedding
# risk_group_mix=np.array([  # Probabilities of partnering with male risk groups for:
#     [0.90, 0.05, 0.05],    # F0
#     [0.05, 0.90, 0.05],    # F1
#     [0.05, 0.05, 0.90],    # F2
# ]),
# risk_assortativity=np.array([20, 10, 5]),
# age_group_bins=[10, 20, 25, 100],  # For separating women into bins and assigning partner age preferences
# age_diff_bins=[-10, 0, 1, 5, 10, 25],  # For binning the preferred age differences between women and men
# am_00=ss.rv_discrete(values=([0, 1, 2, 3, 4], [0.000, 0.003, 0.121, 0.372, 0.504])),
# am_01=ss.rv_discrete(values=([0, 1, 2, 3, 4], [0.000, 0.003, 0.121, 0.372, 0.504])),
# am_02=ss.rv_discrete(values=([0, 1, 2, 3, 4], [0.000, 0.125, 0.750, 0.120, 0.005])),
# am_10=ss.rv_discrete(values=([0, 1, 2, 3, 4], [0.000, 0.003, 0.121, 0.372, 0.504])),
# am_11=ss.rv_discrete(values=([0, 1, 2, 3, 4], [0.000, 0.003, 0.121, 0.372, 0.504])),
# am_12=ss.rv_discrete(values=([0, 1, 2, 3, 4], [0.000, 0.200, 0.500, 0.200, 0.100])),
# am_20=ss.rv_discrete(values=([0, 1, 2, 3, 4], [0.000, 0.003, 0.121, 0.372, 0.504])),
# am_21=ss.rv_discrete(values=([0, 1, 2, 3, 4], [0.000, 0.003, 0.121, 0.372, 0.504])),
# am_22=ss.rv_discrete(values=([0, 1, 2, 3, 4], [0.000, 0.200, 0.500, 0.200, 0.100])),

# age_mix=dict(
#     teens=np.array([                          # Girls aged 15-19
#         [0.000, 0.003, 0.121, 0.372, 0.503],  # F0
#         [0.000, 0.003, 0.121, 0.372, 0.503],  # F1
#         [0.000, 0.100, 0.750, 0.100, 0.005],  # F2
#     ]),
#     young=np.array([                          # Young women aged 20-24
#         [0.000, 0.003, 0.121, 0.372, 0.503],  # F0
#         [0.000, 0.003, 0.121, 0.372, 0.503],  # F1
#         [0.000, 0.200, 0.500, 0.200, 0.100],  # F2
#     ]),
#     adult=np.array([                          # Women aged 25+
#         [0.001, 0.003, 0.121, 0.372, 0.503],  # F0
#         [0.001, 0.003, 0.121, 0.372, 0.503],  # F1
#         [0.000, 0.200, 0.500, 0.200, 0.100],  # F2
#     ]),
# ),

# # Age bins
# f_age_bins = self.pars.age_group_bins
# f_bins = np.digitize(people.age[f_eligible], bins=f_age_bins)-1  # Age bins of individuals
# bin_range_f, males_needed = np.unique(f_bins, return_counts=True)
# age_diffs = self.pars.age_diff_bins
#
# for ab, nm in zip(bin_range_f, males_needed):  # Loop over age bins of females & number of males needed for each
#     for rg in range(self.pars.n_risk_groups):
#         male_dist = self.pars[f'am_{ab}{rg}']  # Refactor
#         f_el_age = ss.true((people.age >= f_age_bins[ab]) & (people.age < f_age_bins[ab+1]) & f_eligible)
#         males = male_dist.rvs(f_el_age)-f_el_age  # Age bins of the male partners


        # if len(m_uids) >= len(f_uids):
        #     n_pairs = len(f_uids)
        #     p.p_pair_form.kwds['p'] = n_pairs/len(m_uids)  # Update the probability of a pair forming
        #     p1 = p.p_pair_form.filter(m_uids)
        #     p2 = f_uids
        # else:
        #     n_pairs = len(m_uids)
        #     p.p_pair_form.kwds['p'] = n_pairs/len(f_uids)  # Update the probability of a pair forming
        #     p1 = m_uids
        #     p2 = p.p_pair_form.filter(f_uids)
# @staticmethod
# def min_age_mismatch(ama, dma, max_iterations=10, increment=20):
#     """ Maybe move to utils """
#     def mse(a, b, i): return ((a.values[i:i+len(dma)] - b.values)**2).mean()
#     si = len(ama)-len(dma)
#     counter = 0
#     mses = []
#     while counter < max_iterations:
#         this_mse = mse(ama, dma, si)
#         mses.append(this_mse)
#         if this_mse == min(mses):
#             si -= increment
#         else:
#             break
#     return si, ama.uid[si:si+len(dma)]
        # import scipy.spatial as spsp
        # import scipy.optimize as spo
        # cost = spsp.distance_matrix(want_ages_sorted.values[:, np.newaxis], ppl.age[m_uids].values[:, np.newaxis])
        # row_ind, col_ind = spo.linear_sum_assignment(cost)

        # import traceback; traceback.print_exc(); import pdb; pdb.set_trace()
        #
        # # Now match lengths
        # if len(m_uids) >= len(f_uids):  # More males: take the older males
        #     n_pairs = len(f_uids)
        #     p1 = m_uids[-n_pairs:]
        #     p2 = f_uids
        # else:  # More females: take the younger females
        #     n_pairs = len(m_uids)
        #     p1 = m_uids
        #     p2 = f_uids[:n_pairs]
        #
        # new_n_pairs = min(len(p1), len(p2))
        # if len(p1) == new_n_pairs: p2 = p2[:new_n_pairs]
        # elif len(p2) == new_n_pairs: p1 = p1[:new_n_pairs]
# dur_commit=dict(
#     teens=dict(
#         f0=ss.lognorm_mean(100, 1),
#         f1=ss.lognorm_mean(8, 5),
#     ),
#     young=dict(
#         f0=ss.lognorm_mean(100, 1),
#         f1=ss.lognorm_mean(10, 5),
#     ),
#     adult=dict(
#         f0=ss.lognorm_mean(100, 1),
#         f1=ss.lognorm_mean(12, 5),
#     ),
# ),
